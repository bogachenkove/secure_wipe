...
OPTIONS
  ...
  Disk discovery:
    -L, --list-storage            List all storage devices (including system disks)
    -S, --select-storage          Interactive storage device selection (shows all disks)
  ...
  Emergency mode (zero‑write only, no full wipe):
    ...
    --resume N                    Resume emergency wipe from sector N (requires --emergency)
...
EXAMPLES
  securewipe -L                                  # list all disks (including system)
  ...
  securewipe --emergency --resume 5000 /dev/sdc  # resume emergency zero‑write from sector 5000
...
NOTES
  ...
  - System disks are shown and can be selected, but a warning is displayed.
  ...
  - Emergency mode is intended for quick partial overwrite (e.g., wiping partition table or boot area).
    ...
    Use --resume to continue a previous emergency wipe from a given sector.
