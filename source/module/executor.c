...
after_analysis:
    ...
    if (cycleNum == 1 && !config->autoConfirm &&
        !confirmWipe(config->devicePath, analysis->totalBytes, config->method, actualPasses))
    {
        printf("Operation cancelled.\n");
        result = ERR_PERMISSION;
        goto cleanup;
    ...
    if (wipeError == ERR_OK)
    {
        ...
        printf("\n--- Wipe complete (cycle %d) ---\nTime: %s\nSectors wiped: %llu\nPasses: %llu\n",
            cycleNum, timeString,
            (unsigned long long)wipeStats->sectorsWiped,
...
int emergencyWipe(const program_config_t* config)
{
    ...
    if (sectorsToWrite > device.sectorCount)
    {
        fprintf(stderr, "Requested %llu sectors, but device has only %llu sectors\n",
            (unsigned long long)sectorsToWrite, (unsigned long long)device.sectorCount);
        deviceClose(&device);
        return 1;
    }
    ...
    while (writtenSectors < sectorsToWrite)
    {
        ...
        printf("\rProgress: %llu / %llu sectors (%.1f%%)",
            (unsigned long long)writtenSectors,
