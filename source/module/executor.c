...
int runWipe (const program_config_t *config, analysis_result_t *analysis)
{
 ...
 for (uint32_t cycle = 1; cycle <= config->cycles; cycle++)
 {
  ...
  if (config->destroyPartitionTable)
  {
   ...
   finalExitCode = (errorCode == ERR_OK) ? 0 : 1;
   goto cleanup_cycle;
  }
 ...
 after_analysis:
  ...
  if (cycle == 1 && !config->autoConfirm && !confirmWipe (config->devicePath, analysis->totalBytes, config->method, actualPasses))
  {
   printf ("Operation cancelled.\n");
   finalExitCode = 1;
   goto cleanup_cycle;
 ...
 cleanup_cycle:
  ...
  if (config->cycles > 1 && cycle < config->cycles && wipeError == ERR_OK)
  {
 ...
 return (finalExitCode == 0 && wipeError == ERR_OK) ? 0 : 1;
}
