        ...
        if (ioOps->writeSectors(dev, currentSector, sectorsToWrite, gWipeBuffer) != ERR_OK)
        {
            ...
            for (uint32_t offset = 0; offset < sectorsToWrite; offset++)
            {
                ...
                if (badSectors && analyzerIsBadSector(badSectors, targetSector))
                    continue;
                if (patternType == PATTERN_RANDOM)
                    randomFill(gWipeBuffer, dev->sectorSize);
...
error_code_t wiperExecute(device_t *dev, const wipe_config_t *config, wipe_stats_t *stats)
{
    ...
    if (stats)
    {
        ...
        if (resultCode == ERR_OK)
            stats->sectorsWiped = dev->sectorCount * stats->totalPasses;
    }
...
static void overwriteMetadataRange(device_t *dev, uint64_t startSector, uint64_t endSector,
                                   int passIndex, progress_callback_t progress,
...
{
    if (!dev || !dev->isOpen || dev->readOnly || passes <= 0)
        return ERR_INVALID_ARG;

    LOG_INFO("Destroying filesystem metadata (%d passes)", passes);
    uint64_t metadataSectors = 8192;
    if (metadataSectors > dev->sectorCount / 2)
        metadataSectors = dev->sectorCount / 2;

    const device_io_ops_t *ioOps = deviceIoGetOps();

    for (int passIndex = 1; passIndex <= passes; passIndex++)
    {
        overwriteMetadataRange(dev, 0, metadataSectors, passIndex, progress, "Destroying metadata (start)");
        if (dev->sectorCount > metadataSectors)
        {
            uint64_t endStart = dev->sectorCount - metadataSectors;
            overwriteMetadataRange(dev, endStart, dev->sectorCount, passIndex, progress, "Destroying metadata (end)");
        }
        ioOps->flush(dev);
    }
...
#else
    int readonlyFlag = 0;
    if (ioctl(dev->handle, BLKROSET, &readonlyFlag) == 0)
        return true;
